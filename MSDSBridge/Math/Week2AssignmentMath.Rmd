...
sg<- 8
pk<- 5
factorial(sg)/(factorial(pk)*factorial(sg-pk))
...
```{r}
fx <- function(N,n){factorial(N)/(factorial(n)*factorial(N-n))}
tp<- 14
pz<- 6
fx(tp,pz)
```
...
```{r}
fy<-function(N,n){factorial(N)/factorial(N-n)}
ch <- 31
dg <- 8
fy(ch, dg)
```
...
Step 4. Find the value of ????????(???????? ê 9).  Round your answer to one decimal place.   
 
```{r}
prs<- s * ps * 2.71828183 * 9
round(prs, digits = 1)

```
 
Step 5. Find the value of ????????(???????? Ç 7).  Round your answer to one decimal place.   

```{r}
c<- s*ps * fx(7,3)
round(c, digits = 1)
```
25.	Suppose a basketball player has made 188 out of 376 free thro*ws.  If the player makes the next 3 free throws, I will pay you $23.  Otherwise you pay me $4. 
...
```{r}
fft <- 994
round((m/fft)*mk+((fft-m)/fft*ms), digits = 2)
```
...
mk<-583
ms<-35
round((m/fft)*mk+(m-1)/(fft-1)*ms, digits = 2)
```
...
e<-2
d<- .3
round(1-pbinom(s*d,s,d), digits = 3)
```
...
e<- 2.7183
s<- 5
round((e^-m*m^s)/factorial(s), digits = 4)
```
...
unr <- 19
d<- 8
((factorial(d)/factorial(ovr))*(ovr/s))
```
