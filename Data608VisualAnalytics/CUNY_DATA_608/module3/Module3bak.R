...
ui1 <- fluidPage(
  headerPanel('Crude Mortality Rate in All States for 2010'),
  sidebarPanel(
...
server2 <- function(input, output, session) {
  ...
  output$plot2 <- renderPlotly({
    ...
    require(plotly)
    p <- ggplot(cdcSlice, aes(Year, DeathRate))+
      geom_point(aes(size = DeathRate, frame = Year, ids = State))+
      scale_fill_continuous()+
      labs(color = "State", x = "Years", y = "DeathRate %Change")+
      geom_line()
