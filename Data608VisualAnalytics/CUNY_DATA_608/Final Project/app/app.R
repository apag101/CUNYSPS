...
ui1 <- fluidPage(
  ...
  mainPanel(
    ...
    tabPanel("GLM Summary",verbatimTextOutput('stats')),
...
server1 <- function(input, output, session) {
  ...
  summary(glm(cdc$sprtcode~cdc[,names(cdc) %in% input$colr]|cdc[,names(cdc) %in% input$colp],family="binomial"(link="logit")))
