...
```{r message=FALSE, warning=FALSE}
lbls <- paste(tddata1.2$County, round(tddata1.2$pctallcrime,0)) 
lbls <- paste(lbls,"%",sep="") 
...
tddata2$total<- as.numeric(tddata2$total)
...
```{r message=FALSE, warning=FALSE, fig.width=10, fig.height=6}
require(plotly)
p <- ggplot(tddata2, aes(total, year, color = CntName)) +
  geom_point(aes(size = total, frame = year, ids = CntName))+
  scale_fill_continuous()
    
p <- ggplotly(p) %>%
    animation_opts(
    2000, easing = "circle", redraw = TRUE
  )
p
# p = NULL
```
...
tddata3$count<- as.numeric(tddata3$count)
...
```{r message=FALSE, warning=FALSE}
ggplot(data= tddata3) +
    geom_bar(mapping = aes(x = airline, y = count, fill= airline), stat = "identity", position = "identity", show.legend = FALSE)+
theme(axis.text.x=element_text(angle=-90))
