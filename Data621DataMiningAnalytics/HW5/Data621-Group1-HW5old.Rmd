...
WinesTrain <- read.csv("wine-evaluation-data.csv",header=TRUE)
WinesEval <- read.csv("wine-training-data.csv",na.strings="",header=TRUE)
...
wines1 <- describe(WinesTrain)
wines1$na_count <- sapply(WinesTrain, function(y) sum(length(which(is.na(y)))))
wines1$na_count_perc <- sapply(WinesTrain, function(x) round(sum(is.na(x))/nrow(WinesTrain)*100,1))
```


```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
colsTrain<-ncol(WinesTrain)
colsEval<-ncol(InsEval)
missingCol<-colnames(WinesTrain)[!(colnames(WinesTrain) %in% colnames(InsEval))]
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
cc<-summary(complete.cases(WinesTrain))
cWinesTrain<-subset(WinesTrain, complete.cases(WinesTrain))
cc
```

With these results, if we remove all rows with incomplete rows, there will be a total of `r as.integer(cc[3])` rows out of `r nrow(WinesTrain)` .If we eliminate all non-complete rows and keep only rows with data for all the predictors in the dataset, our new dataset will results in `r round(as.integer(cc[3])/nrow(WinesTrain),2)*100`% of the total dataset. We create a subset of data with complete cases only to use later in our analysis.
...
```{r echo=FALSE,message=FALSE,warning=FALSE}
#sapply(WinesTrain, function(x) round(sum(is.na(x))/nrow(WinesTrain)*100,1))
vis_miss(WinesTrain)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
cat(colnames(WinesTrain[ sapply(WinesTrain, is.factor)]), "\n\n")
glimpse(WinesTrain)
```
...
```{r}
sapply(WinesTrain, function(x) round(sum(is.na(x))/nrow(WinesTrain)*100,1))
WinesTrain[,r] <- replace_na(WinesTrain[,r], as.list(colMeans(WinesTrain[,r], na.rm = TRUE)))
...
```{r}
m1<-glm(TARGET_FLAG~.-INDEX-TARGET_AMT,data=WinesTrain,family="binomial"(link="logit"))
summary(m1)
```
...
g<-roc(TARGET_FLAG~targethat,data=WinesTrain)
plot(g)
```
...
table(targethat,WinesTrain$TARGET_FLAG)
```
...
```{r echo=FALSE,message=FALSE,warning=FALSE}
InsMut <- mutate(WinesTrain, Residuals = residuals(m3), linPred = predict(m3))
...
```{r echo=FALSE,message=FALSE,warning=FALSE}
linPred <- predict(m3)
InsMut <- mutate(WinesTrain, predProb = predict(m3, type = "response"))
grpIns <- group_by(InsMut, cut(linPred, breaks = unique(quantile(linPred, (0:25)/26))))
...
g<-roc(TARGET_FLAG~targethat,data=WinesTrains)
plot(g)
```
...
targethat[targethat<0.5]<-0
targethat[targethat>=0.5]<-1
table(targethat,WinesTrains$TARGET_FLAG)
```
...
```{r echo=FALSE,message=FALSE,warning=FALSE}
InsMut <- mutate(WinesTrains, Residuals = residuals(m4), linPred = predict(m4))
grpIns <- group_by(InsMut, cut(linPred, breaks=unique(quantile(linPred, (0:25/26)))))
diagIns <- summarise(grpIns, Residuals = mean(Residuals), linPred = mean(linPred))
plot(Residuals ~ linPred, data = diagIns, xlab="Linear Predictor")
abline(h = 0, lty = 2, col = "darkorange", lwd = 2)
...
```{r echo=FALSE,message=FALSE,warning=FALSE}
linPred <- predict(m4)
InsMut <- mutate(WinesTrains, predProb = predict(m4, type = "response"))
grpIns <- group_by(InsMut, cut(linPred, breaks = unique(quantile(linPred, (0:25)/26))))
#hosmer-lemeshow stat
hlDf <- summarise(grpIns, y= sum(TARGET_FLAG), pPred=mean(predProb), count = n())
hlDf <- mutate(hlDf, se.fit=sqrt(pPred * (1-(pPred)/count)))
...
```{r}
WinesTrain<-WinesTrain[ , !(names(WinesTrain) %in% c('predicted_m3','target_m4'))]
lm1<-lm(TARGET_AMT~.-TARGET_FLAG,WinesTrain)
summary(lm1)
par(mfrow = c(2,2))
plot(lm1)
```
...
```{r}
lm2 <- lm(TARGET_AMT ~ +AGE +EDUCATION +REVOKED +MVR_PTS +JOB +YOJ +CLM_FREQ +HOME_VAL +URBANICITY +PARENT1 +MSTATUS +TRAVTIME +BLUEBOOK, data = WinesTrain)
summary(lm2)
par(mfrow = c(2,2))
plot(lm2)
```
...
```{r}
lm4<-rlm(TARGET_AMT~.-TARGET_FLAG,WinesTrain,maxit=40)
summary(lm4)
par(mfrow = c(2,2))
plot(lm4)
```
