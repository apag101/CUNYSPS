...
We build a Hubber weigthed model to account for the distant points observed in the previous models.

```{r}
InsTrains <-subset(InsTrain, PARENT1='YES',CAR_TYPE %in% c('Minivan','Pickup','Sports Car','Van','z_SUV'), REVOKED = 'YES', CAR_USE = 'Private')
lm4<-rlm(TARGET_FLAG ~ ., InsTrains,maxit=40)
summary(lm4)
par(mfrow = c(2,2))
plot(lm4)
```

The models doesn't seem to help with weigthed coefficients, we stil see the effects of several datapoints in the data.
...
For both the logistic regression and linear regressions we picked model 3. Results for the logistic regression were rather good, but the linear regression doesn't seem to be a good model, even when using weigthed coefficients. 
