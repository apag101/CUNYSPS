...
def exercise01():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df_first_five_rows = pd.DataFrame(data=iris.data, columns=iris.feature_names).iloc[:5]
...
def exercise02(new_observations):
    ...
    iris_predictions =iris.target_names[knn.predict(new_observations)]
    # ------ Place code above here /\ /\ /\ ------
...
def exercise04():
    ...
    data = [trace]
    plotly_overfit_underfit_curve_url = py.plotly.iplot(data, filename = 'overfit_underfit_curve_url' , auto_open=True)   
    # ------ Place code above here /\ /\ /\ ------
...
def exercise05():
    ...
    df.target = pd.DataFrame(data=boston.target)
    average_age =boston.data[:,6].mean()
    average_medv = boston.target.mean()
...
def exercise06():
    ...
    # ------ Place code below here \/ \/ \/ ------
    plt.xlabel('PTRATIO')
    plt.ylabel('Value of homes')
    plt.scatter(boston.data[:,10],boston.data[:,6])
    X_ptratio = np.array(boston.data)[:,10]
    # ------ Place code above here /\ /\ /\ ------
    return X_ptratio
...
def exercise07():
    ...
    reg_model = reg_model.predict(prediction_space)
    plt.xlabel('PTRATIO')
    plt.ylabel('Value of homes')
