...
# Change password
def get_credentials():
    return {'user':'root','password':'123456'}
...
class Iris:
    def __init__(self,creds,dbname='data602',new=True):
        ...
        if new:
            ...
            cursor.execute('USE '+self.__dbname+';')
    ...
    # Execute the create database SQL statment through the cursor instance        
        sql = 'DROP DATABASE IF EXISTS ' + self.__dbname +';'
        sql +='CREATE DATABASE ' + self.__dbname +';'
        sql += 'USE ' + self.__dbname+';'
        sql += 'CREATE TABLE iris_data(id INT NOT NULL,feature_sepal_length FLOAT NOT NULL,feature_sepal_width FLOAT NOT NULL,feature_petal_length FLOAT NOT NULL,feature_petal_width FLOAT NOT NULL,target_species VARCHAR(20) NOT NULL,target_species_id INT NOT NULL);'     
        cursor.execute(sql)
        cursor.close()
    ...
    def load(self,truncate=False):
        con = Iris.__get_connection(self, get_credentials())
        cursor = con.cursor()
        cursor.execute('USE '+self.__dbname+';')
        iris = ds.load_iris()
        df = pd.DataFrame(data=iris.data, columns=iris.feature_names)
        target_names = pd.DataFrame(data={'id':df.index,'target_species':iris.target_names[iris.target],'target_species_id':iris.target})
        df = pd.concat([df, target_names], axis=1)
        if truncate:
            # ------ Place code below here \/ \/ \/ ------
        ...
        # ------ Place code below here \/ \/ \/ ------            
        sql = "INSERT INTO " +self.__dbname+ ".iris_data(feature_sepal_length,feature_sepal_width,feature_petal_length,feature_petal_width,id,target_species,target_species_id) VALUES (%s,%s,%s,%s,%s,%s,%s)"
        for d in range(len(df)):
            cursor.execute(sql, list(df.values[d]))
            con.commit()
        cursor.close()
        # ------ Place code above here /\ /\ /\ ------
    ...
    def display_gt(self,n): 
        ...
        cursor = con.cursor()
        cursor.execute('USE '+self.__dbname+';')
        cursor.execute('select * from iris_data where id >' +str(n))
    ...
    def update_observation(self,id,new_target_species,new_target_species_id):
        ...
        cursor = con.cursor()
        cursor.execute('USE '+self.__dbname+';')
        cursor.execute("update iris_data set target_species_id ='%s',target_species ='%s' where id = '%s'" %(str(new_target_species_id), new_target_species,str(id)))
        con.commit()
        cursor.close()     
        # ------ Place code above here /\ /\ /\ ------
    ...
    def del_observations(self,row_ids):
        ...
        cursor.execute('USE '+self.__dbname+';')
        for d in row_ids:
            cursor.execute('delete from iris_data where id =' +str(d))
            con.commit()       
        cursor.close()
        # ------ Place code above here /\ /\ /\ ------
