# -*- coding: utf-8 -*-
"""
Created on Sat Apr  6 21:48:36 2019
...
class PandasChain:
    ...
    def __commit_block(self,block): 
        ...
        self.__merkle_tx_hash = self.__id
        block_hash = Block.set_block_hash(block,self.__id)# Create block hash
        self.__prev_hash = self.__current_block.hash 
        self.seq_id = self.__seq_id + 1
        # Add code here
        self.__chain.append(Block(block_hash, self.__prev_hash))
    ...
    def get_values(self):
        for i in self.__chain.__iter__():
            return self.__chain.__getitem__(i)
        pass
...
class Block:
    ...
    def __init__(self,seq_id,prev_hash): 
        ...
        self.__transactions =  pd.DataFrame(data={'Timestamp':[],'Sender':[],'Receiver':[],'Value':[],'TxHash':[]})# Create a new blank DataFrame with set headers
        self.__status = 'UNCOMMITTED'# Initial status. This will be a string.
        self.__merkle_tx_hash = None
        self.__block_hash = hashlib.sha256(str(str(uuid.uuid4())+str(self.__seq_id)+str(prev_hash)+str(seq_id)+str(self.__merkle_tx_hash)+str(dt.datetime.now())).encode('utf-8')).hexdigest()
    ...
    def display_header(self): 
        ...
        #print(self.__seq_id, self.__status, self.__block_hash,  self.__prev_hash, self.__merkle_tx_hash, len(self.display_transactions))
        pass
    
    # 10 pts - This is the interface for how transactions are added
    def add_transaction(self,s,r,v): 
        ts = str(dt.datetime.now())# Get current timestamp 
        tx_hash = self.__block_hash # Hash of timestamp, sender, receiver, value
        new_transaction = pd.DataFrame(data={'self':self, 'Sender':[s],'Receiver':[r],'Value':[v],'Timestamp':[ts],'TxHash':[tx_hash]}) # Create DataFrame with transaction data (a DataFrame with only 1 row)
    ...
    def get_values(self):
        return self.__col_names.__getitem__.__dict__.values()
        pass
