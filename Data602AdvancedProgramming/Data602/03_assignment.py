...
class Box:
    ...
    def __eq__(self, other):
        if other.__length == self.__length and other.__width == self.__width:
            return True
        else:
            return False
    ...
    def combine(self, other):
        l = other.__length + self.__length
        w = other.__width + self.__width
        print('*' * w)
        print(('*'+ ' ' * (w -2) + '*\n')*(l-2))
        print('*' * w)
...
class MangoDB:
    ...
    def display_all_collections(self):
        for c in range(len(self.collections)):
            print('collection: ' +  self.collections[c])
    ...
    def wipe(self):
        ...
        #for c in range(len(self.collections)):
            #if 'default' not in self.collections[c]:
                #self.collections.pop([c])
...
'''
    ...
    box2.print_dim()
    box3.print_dim()
    Box.__eq__(box1,box2)
    Box.__eq__(box1,box3)
    box3.combine(box1)
    box2.double()
    box2.combine(box1)
...
def exercise02():
    ...
    # ------ Place code below here \/ \/ \/ ------
    m = MangoDB()
    m.add_collection('testscores')
    for i in range(len(test_scores)):
        m.update_collection('testscores', {i+1:test_scores[i]})
    print(m.get_collection_size('testscores'))
    m.list_collection()
    print(m.uid)
    m.wipe()
    m = MangoDB()
