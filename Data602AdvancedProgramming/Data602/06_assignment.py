...
py.tools.set_credentials_file(username='apag101', api_key='6Uv0pAnLKGGpLqaj30iL')
...
def exercise01():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df = pd.DataFrame(data={0:[1,5,7],1:[2,6,8],2:[3,7,9],3:[4,8,10]})
    # ------ Place code above here /\ /\ /\ ------
...
def exercise04(a):
    ...
    # ------ Place code below here \/ \/ \/ ------
    #sum = np.sum(a)[1]
    sum = np.sum(np.array(a).reshape(-1,2)[::1,[1]])
    # ------ Place code above here /\ /\ /\ ------
    return sum
...
def exercise08():
    ...
    row_count = len(df)
    avg_sq_ft = df.mean()[3]
    df_zip_95670 = df.loc[df['zip'].isin(['95670'])]
    df_zip_not_95610 = df.loc[~df['zip'].isin(['95610'])]
    # ------ Place code above here /\ /\ /\ ------
...
def exercise09():
    ...
    resp = requests.get(url=url, params=header)
    df = pd.DataFrame(resp.json()).iloc[:-2,[0]]
    data = [go.Scatter(x=df.index, y=df.bpi)]
    plotly_url = py.plotly.iplot(data, filename = 'time-series-plot' , auto_open=True)
    print(df.head())
    print(df.tail())
    # ------ Place code above here /\ /\ /\ ------    
...
def exercise11(n):
    ...
    array_reshaped = np.resize(array_1d,(3,int(n/3)))
    # ------ Place code above here /\ /\ /\ ------  
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix=np.array(([1,0]*n+[1,0]*n)*n).reshape((2*n,2*n))
    # ------ Place code above here /\ /\ /\ ------ 
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    s=pd.DataFrame({'s': pd.Series(np.random.randint(0,n, size=n), index=pd.date_range('1/1/2010', periods=n))})
    s = plt.plot(s.apply(np.cumsum))
    # ------ Place code above here /\ /\ /\ ------ 
    return s
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.Series(map(lambda x:len(x), words))
    # ------ Place code above here /\ /\ /\ ------ 
    return df
