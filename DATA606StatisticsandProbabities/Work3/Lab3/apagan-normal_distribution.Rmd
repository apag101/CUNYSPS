...
subtitle: "Lab3 Completed (Anthony Pagan)"
output:
  html_document:
    theme: cerulean
    highlight: pygments
...
####One thing woud be to compare the SD of each.
...
####Yes, the plot appears to be normally distributed
...
#### The sim_norm is similar to the real data. Both fall within the line and have similar shapes.
```{r}
qqnorm(sim_norm)
qqline(sim_norm)
```
...
#### Yes the normal plots appear to be similar to real data which shows evidence that women heights are nearly normal.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    
####Based on Graphs below the real data is different from random data. Random datat looks to be more on the line then the real data.
...
sim_wnorm <- rnorm(n = length(fdims$wgt), mean = fhgtmean, sd = fwgtsd)
qqnorm(sim_wnorm)
qqline(sim_wnorm)
qqnormsim(fdims$wgt)
```
...
#### Percent of women heights 1 standard deviations above the mean
```{r}
1-pnorm(q = fhgtmean+(fhgtsd), mean = fhgtmean, sd = fhgtsd)
sum(fdims$hgt > fhgtmean+(fhgtsd)) / length(fdims$hgt)
```

#### Percent of women heights 1 standard deviations below the mean
```{r}
1-pnorm(q = fhgtmean-(fhgtsd), mean = fhgtmean, sd = fhgtsd)
sum(fdims$hgt > fhgtmean-(fhgtsd)) / length(fdims$hgt)
```
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **a.** The histogram for female biiliac (pelvic) diameter (`bii.di`) belongs
    to normal probability plot letter _C_
    
```{r}

sim_wnorm1 <- rnorm(n = length(fdims$wgt), mean = mean(fdims$bii.di), sd = sd(fdims$bii.di))
qqnorm(sim_wnorm1)
qqline(sim_wnorm1)
...
sim_wnorm2 <- rnorm(n = length(fdims$wgt), mean = mean(fdims$elb.di), sd = sd(fdims$elb.di))
qqnorm(sim_wnorm2)
qqline(sim_wnorm2)
...
sim_wnorm3 <- rnorm(n = length(fdims$wgt), mean = mean(fdims$age), sd = sd(fdims$age))
qqnorm(sim_wnorm3)
qqline(sim_wnorm3)
...
```
    ...
    **d.** The histogram for female chest depth (`che.de`) belongs to normal 
    probability plot letter __D__.
    
    
```{r}

sim_wnorm4 <- rnorm(n = length(fdims$wgt), mean = mean(fdims$che.de), sd = sd(fdims$che.de))
qqnorm(sim_wnorm4)
qqline(sim_wnorm4)
```
...
####C is more normally distributed, D is slightly skewed to the right.
...
```{r}
sim_wnorm5 <- rnorm(n = length(fdims$wgt), mean = mean(fdims$kne.di), sd = sd(fdims$kne.di))
qqnorm(sim_wnorm5)
qqline(sim_wnorm5)
