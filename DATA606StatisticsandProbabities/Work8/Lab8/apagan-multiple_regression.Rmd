...
####This is an observational study as it is takes information from collected data observed to draw a conclusion. It tries to establish an association between a explanatory and a response variable. The question should be is beatuy associated with the difference in course evaluations.

2.  Describe the distribution of `score`. Is the distribution skewed? What does 
    that tell you about how students rate courses? Is this what you expected to 
    see? Why, or why not?
    
####The distribution of score is left skewed. This tell us that most students who filled out the survey are evaluating the teachers at the higher rate regardless of beauty. I would expect this to be the case, although the survey is subjective and depends on the students preference.
...
####The age and relation to evaluatoin appears to have a negative correlation. The older the professor the lower the rating. 
...
#### Yes there appears to be some NULLs in the data.
...
####It appears to be highlighting some data points, but not all.
    
```{r scatter-score-bty_avg_jitter, eval = FALSE}
plot(jitter(evals$score, factor = 1, amount = NULL) ~ evals$bty_avg)
```
...
####Lnear equation is y = `r m_bty$coefficients[1]` + `r m_bty$coefficients[2]` x  The average score changes by `r m_bty$coefficients[2]` for every beauty score. It does not appers to be a signficant predictor.
...
####The conditions of least square are not reasonable as the R Squared is only 3% so the proportion of variability explained by this variable is only 3%.
...
####The residuals history appear to be slightly skewed to the left , the variabilitly seems higher close to even and qqplot is nearly norml.
...
#### Bty_avg does still appear to be a significant predictor of score.
...
####The equation is y = `r m_bty_gen$coefficients[1]` + m_bty_gen$coefficients[3]` x. The males tend ot have the higher course evaluation score.
...
####It separates the ranks, but looks like it drops the first rank, teaching.
    
```{r scatter-score-bty_ran, eval = FALSE}
...
####I would say number of credits will have the highest p value
...
```{r m_credits, eval = FALSE, tidy = FALSE}
...
#### y = `r m_ethnicity$coefficients[1]` + `r m_ethnicity$coefficients[2]`x. The enthnity raises score by `r m_ethnicity$coefficients[2]` for each score point.
...
m_full_d <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs +  bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_full_d)
```

####The y interecept goes from 4.09 to 4.39 after dropping the credit variable. 
...
```{r m_full_d, eval = FALSE, tidy = FALSE}
m_full_d <- lm(score ~ rank + gender + language + age + cls_perc_eval 
             + pic_outfit + pic_color, data = evals)
summary(m_full_d)
``` 

####Best model is rank + gender + language + age + cls_perc_cal + pic_outfit + pic_color

16. Verify that the conditions for this model are reasonable using diagnostic 
    plots.
    
####Model appears to be nearly normal with balanced variability
    
```{r eval = FALSE}
plot(evals$score ~ m_full_d$residuals)
hist(m_full_d$residuals)
qqnorm(m_full_d$residuals)
qqline(m_full_d$residuals)
...
####Yes it can have an impact, as the type of course and the professors ability to teach the course can have an impact on score. 

18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score.
    
#### Based on the final model, a professor with a higher evaluatoin would score high high on  ranking , be mostly male, young, with high percent of class completing survey, with a picture with high quality color wearing high quality outfit and speaks same language as students in school.

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?
    
####I would not be confortable generalizing to any professors general as this data is specific to this university and correlation does not equate to causation.
