...
subtitle: "Lab5 Completed (Anthony Pagan)"
output:
  html_document:
    css: ./lab.css
    highlight: pygments
...
####There are `r nrow(nc)` cases in this dataset
...
####Smokers and Non-smoker weight are about same average, although non-smokers have more to the left with more at lower weights.

```{r}
library(ggplot2)
ggplot(nc, aes(habit, weight, na.rm = TRUE)) + geom_boxplot() +
  ylab("Weight") 
...
####Ye the sample sizes are large enough to be random, independent and normalized
...
####Null hypothesis states that average weights of smoking vs non-smoking is not different. The Alternateive hypothesis is the average weights are different
...
```{r}
inference(y = nc$weeks, est = "mean", type = "ci", conflevel = 95,  null = 0, alternative = "twosided", method = "theoretical")
```
...
```{r}
inference(y = nc$weeks, est = "mean", type = "ci", conflevel = 90,  null = 0, alternative = "twosided", method = "theoretical")
```
...
mean_mature <- 7.1256
sd_mature <-1.6591
mmse<- sd(mm$weeks, na.rm =TRUE)/sqrt(length(mm))
mean_younger <- 7.0972
sd_younger <-1.4855
ymse<- sd(ym$weeks, na.rm =TRUE)/sqrt(length(ym))

mmlower <- mean_mature - 1.96 * mmse
mmupper <- mean_mature + 1.96 * mmse
round(c(mmlower, mmupper),2)

ymlower <- mean_younger - 1.96 * mmse
ymupper <- mean_younger + 1.96 * mmse
round(c(ymlower, ymupper), 2)
```

####The range with 95% confidence for mature mothers is `r round(c(mmlower, mmupper),2)` and the range for yonger mothers is `r round(c(ymlower, ymupper),2)`
...
####Comparing the number of visits to the lowbirth weights

```{r}
inference(y = nc$visits, x = nc$lowbirthweight, est = "mean", type = "ht", null = 0, conflevel = 95,
          alternative = "twosided", method = "theoretical")
```
