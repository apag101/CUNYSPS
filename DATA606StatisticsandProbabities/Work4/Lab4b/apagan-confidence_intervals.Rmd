...
subtitle: "Lab4b Completed (Anthony Pagan)"
output:
  html_document:
    css: ./lab.css
    highlight: pygments
...
#### A typical size of the sample is 1600. The typical size is based on the average of all 60 samples of the population
...
#### Another student's distribution will no be the same, but will b similar. We are comparing sizes from the same population. So on average they will be about the same.
...
####For this to be true the samples must be random and it must be a large enough sample greater than 30.

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

#### 95% confidence says we are 95% confident that a sample would be with a specifc lower and upper range value.
...
####Yes the true avg size falls within the 1480 and 1720 range
...
#####The expectation is that 95% of the sample intervals would capture the true population mean because the samples collectively make up the population.
...
#### `r 46/50*100`% of the confidence interval includes the true population mean. The proportion is not exactly equal probably because the number of samples are not enough or it is part of the 5% of samples that would not be exact.
...
How does this percentage compare to the confidence level selected for
    the intervals?

####Confidence level comes out same as percentage at 99% confidence interval
