...
subtitle: "Lab4a Completed (Anthony Pagan)"
output:
  html_document:
    css: ./lab.css
    highlight: pygments
...
####The distribution looks evenly distributed, but slightly skewed to th right.
...
#### Similarly this sample is slightly skewed to the right.
```{r}
hist(samp1)
```
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    mean of `samp2` compare with the mean of `samp1`? 

#### Samp1 mean is within ~120 more that samp2
...
#### As the sample size grows large the mean estimate will be more accurate estimate of the populatio.
...
#### There are 5000 samples of 50 random houses so there 5000  elements. I would expect the distribution to become more an more evenly distributed as we continue to increase the samples.
...
```{r loop-again, eval=FALSE, message=FALSE, warning=TRUE}
sample_means50 <- rep(NA, 5000)
...
#### There are 100 elements in the sample. Each element is the mean of 50 samples. A sample of 50 was taken 100 times.
...
####When the sample is larger the center gets center becomes denser as spread is sqeazed to the middle.
...
####The sampling distribution is evenly distrubted. The mean home prices are probably ~180000
...
####The sales homes prices should be similar. With a bigger sample size i'd expect prices to be closer to the mean than the sample of 50. We should prefer a large sample if we want it closer to the true value.
