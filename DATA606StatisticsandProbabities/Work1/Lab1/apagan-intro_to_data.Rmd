...
subtitle: "Lab1 Completed (Anthony Pagan)"
output:
  html_document:
    theme: cerulean
    highlight: pygments
...
* exerany num discrete
* hlthplan num discrete
* smoke100  num discrete
* height num continuous
* weight num continuous
* wtdesire discrete
* age num discrete
* gender categorical
...
The mosaic plot reveals that men smoke slight more than women
...
The second histogram is more finer and specific results, you can get closer to details.
...
The weight and weight desired has a positive correlation. This makes sense since the higher the weights the more weight a person would want to lose. C luster of weights are between 100 - 200 pounds
```{r}
library(car)
...
wdiff is a discrete number. If wdiff is 0 then the person is happy with their weight. If wdiff is postive they want to gain weight, if minus then want to lose weight.
...
The histogram shows majority of people want to lose weight. The box plot shows there are many the will like to loose more weight than average but not enough to make it left skewed by a lot.
...
Other than then the fact that women weigh less, the data shows that both men and women have same porportional weight desires.
...
mn <-mean(cdc$weight)
wsd <-sd(cdc$weight)
nrow(subset(cdc, cdc$weight < mn + wsd & cdc$weight > mn - wsd))/nrow(cdc)
```
<div id="license">
