...
#### I would use a scatterplot to show relationship of runs to one of the the the other variables
...
#### Based on below, the at_bats vs bat_avg seem to have a positive upward trend. This predicts the more at bats correlate with higher more runs. I would not be comfortable using this as a predictor as there appears to be a lot of outliers.
...
#### The runs to at bats have a slight positive correlation. Based on the .61 correlation it is a weak positive correlation. There are also several noticable outliers far away from the mean.
...
#### I only get the original numbers. The option to select points does not work.
...
4.  Fit a new model that uses `homeruns` to predict `runs`. Using the estimates 
    from the R output, write the equation of the regression line. 
...
#### The slope tells us there is a positive correlation between homeruns and runs. The correlation value of .79 shows it is a slitghly strong positive correlation.
...
####You would expect `r round((-2789.2429 + 0.6305 * 5578),0)` runs in 5578 at-bats. This is an overestimation by ~16 runs.

```{r}
plot(m2$residuals ~ mlb11$runs)
abline(h = 0, lty =3)
```
## Model diagnostics
...
#### This appears to be evenly distributed on both sides of the residual line so i expect it to be linear. 
...
#### Based on historgram and q plot the nearly normal conditions are met. Historgram appears to be right skewed, but it is nearly normal 
...
####Yes the constant variability condition appear to be met. The residuals are about the on plus and negative side for the least squqred line.
...
#### Batting average appear to also have a postitive correlation t runs. At .80 correlation it is a strong postive correlation.
...
plot_ss(y = mlb11$bat_avg, x = mlb11$runs)
...
####Batting average has as strong positive correlation and high R squared. This shows batting as a better preditor of runs than at bats. 
...
####Strikeouts have a strong negative correlation. The lower the strikeouts the higher the runs.
...
plot_ss(y = mlb11$strikeouts, x = mlb11$runs)
```
...
#### Based on qplots below OBS appears to be the bes predictor of runs as the points are closest to residual lines. A correlation of .97 confirms this.

```{r}
qqnorm(summary(lm(mlb11$runs~mlb11$new_onbase))$residuals)
qqline(summary(lm(mlb11$runs~mlb11$new_onbase))$residuals)

qqnorm(summary(lm(mlb11$runs~mlb11$new_slug))$residuals)
qqline(summary(lm(mlb11$runs~mlb11$new_slug))$residuals)

qqnorm(summary(lm(mlb11$runs~mlb11$new_obs))$residuals)
qqline(summary(lm(mlb11$runs~mlb11$new_obs))$residuals)
...
abline(m4)
cor(mlb11$runs, mlb11$new_obs)
plot_ss(y = mlb11$new_obs, x = mlb11$runs)
```
